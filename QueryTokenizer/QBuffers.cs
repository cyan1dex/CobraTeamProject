...
{
   public class QCharBuffer
   {
      public QCharBuffer(StreamReader sr, int depth)
      {
      ...
      public QCharBuffer(string s, int depth) //Depth is passed in which a length of 2 initially
      {
         buf = new int[depth];	//buffer to hold tokens
         Code = s;
   ...
   public class QTokenBuffer
   {
      public QTokenBuffer(QLexer lex, int k)
      {
         this.k = k;
         buf = new QToken[k];
         lexer = lex;
      ...
      public QToken tokenValue(int i)
      {
         if (i >= 1 && i <= k)
         {
            return buf[i - 1];
